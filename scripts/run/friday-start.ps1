...
function Start-FridayServices {
    ...
    $profiles = @()
    # HA/MQTT só se HA_ENABLED=true no ambiente ou config local
    $haEnabled = $env:HA_ENABLED
    if (-not $haEnabled -and (Test-Path (Join-Path $RepoPath ".env"))) {
        $line = Get-Content (Join-Path $RepoPath ".env") -ErrorAction SilentlyContinue |
            Where-Object { $_ -match '^\s*HA_ENABLED\s*=' } |
            Select-Object -First 1
        if ($line -match '=\s*(.+)$') { $haEnabled = $Matches[1].Trim().Trim('"') }
    }
    if ($haEnabled -in @("1", "true", "True", "TRUE", "yes", "YES")) {
        $profiles += @("--profile", "mqtt", "--profile", "homeassistant")
        Write-Host "Profiles Fase 3: mqtt + homeassistant (HA_ENABLED)"
    }
    # Frigate NUNCA automatico — requer FRIGATE_ENABLED=true e Start-FrigateConfirm (futuro)
